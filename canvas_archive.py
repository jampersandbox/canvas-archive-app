...
def _get_data_dir() -> Path:
    if getattr(sys, "frozen", False):
        if sys.platform == "darwin":
            d = (Path.home() / "Library"
                 / "Application Support" / "Canvas Archive")
        elif sys.platform.startswith("win"):
            d = (Path(os.environ.get("APPDATA", str(Path.home())))
                 / "Canvas Archive")
        else:
...
def _playwright_browsers_dir() -> Path:
    """
    The directory where Playwright browsers are stored.
    Always use the user's cache — never inside the app bundle.
    """
    if sys.platform == "darwin":
...
def write_canvas_config(canvas_url: str, panopto_url: str) -> None:
    ...
    # Write .json for canvas_auth.py which reads without importing
...
def install_browser_dialog(parent) -> bool:
    ...
    tk.Label(
        win, textvariable=sv,
        font=("Helvetica", 10), fg="#555", bg="#f0f0f0",
    ).pack()
...
class CanvasArchiveApp:
    ...
    def _build_ui(self):
        ...
        self._canvas_w = tk.Canvas(
            sc, bg="#f0f0f0", highlightthickness=0
        )
        sb = ttk.Scrollbar(
            sc, orient="vertical", command=self._canvas_w.yview
        )
        ...
        self.main = tk.Frame(
            self._canvas_w, bg="#f0f0f0", padx=20, pady=10
        )
    ...
    def _on_frame_conf(self, e):
        self._canvas_w.configure(
            scrollregion=self._canvas_w.bbox("all")
        )
    ...
    def _on_scroll(self, e):
        if e.num == 4:
            ...
            self._canvas_w.yview_scroll(
                int(-1 * (e.delta / 120)), "units"
            )

    def _build_settings(self):
        sf = ttk.LabelFrame(
            self.main, text=" Settings ", padding=10
        )
        ...
        tk.Label(
            r2, text="Save files to:", width=13,
            anchor="w", bg="white", font=("Helvetica", 11),
        ).pack(side="left")
        ttk.Entry(
        ...
        ttk.Button(
            r2, text="Browse…", command=self._browse
        ).pack(side="left", padx=4)
    ...
    def _build_options(self):
        of = ttk.LabelFrame(
            self.main, text=" Options ", padding=10
        )
    ...
    def _run_next_script(self):
        ...
        # KEY: set PLAYWRIGHT_BROWSERS_PATH so scripts find the browser
        env = os.environ.copy()
...
def main():
    ...
    # First-run browser check — download Chromium if not present
...
if __name__ == "__main__":
    main()        elif sys.platform.startswith("win"):
            data_dir = os.path.join(
                os.environ.get("APPDATA", os.path.expanduser("~")),
                "Canvas Archive",
            )
