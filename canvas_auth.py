...
def _ensure_browser() -> None:
    ...
    PLAYWRIGHT_BROWSERS_PATH should already be set by the parent process
    (canvas_archive.py). If not, fall back to downloading.
    """
    pw_path = os.environ.get("PLAYWRIGHT_BROWSERS_PATH", "")

    if pw_path and Path(pw_path).exists():
        # Check if chromium is actually there
        if any(Path(pw_path).glob("chromium*")):
            log.info(f"  Browser found at: {pw_path}")
            return

    # Not found — download it
    log.info("  Downloading browser (one time only)…")
